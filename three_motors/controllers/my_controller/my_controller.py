...
class Nao (Robot):
    ...
    def setArmAngle(self, angleShoulderPitch, angleShoulderRoll,  angleElbowRoll):
        """
    ...
    def hebbianTest(self, choice):
        ...
        for i in range (max_iterations):
         # Generate random angles within the specified range
            ...
            motor_entry = [randomShoulderPitch, randomShoulderRoll, randomElbowRoll]
    ...
    def MoveArm(self, rotation_angles):
        ...
        while robot.step(self.timeStep) != -1:
            self.setArmAngle(rotation_angles[0], rotation_angles[1], rotation_angles[2])
            #time.sleep(1)
            
            if (abs(self.RShoulderPitchPS.getValue()-self.RShoulderPitch.getTargetPosition()) < motor_tolerance) and (abs(self.RShoulderRollPS.getValue()-self.RShoulderRoll.getTargetPosition()) < motor_tolerance) and (abs(self.RElbowRollPS.getValue()-self.RElbowRoll.getTargetPosition()) < motor_tolerance):
                break
        ...
        with open("motor_angles.csv", "w",newline='') as  motor_csvfile, \
            ...
            for i in range (max_iterations):
                try:
                    ...
                    rotation_angles=[randomShoulderPitch, randomShoulderRoll, randomElbowRoll]
                    # Set the random angles using the function
                    self.MoveArm(rotation_angles)
                    ...
                    motor_writer.writerow([i, randomShoulderPitch, randomShoulderRoll,  randomElbowRoll])
...
def generateAnglesSOM():
    ...
    # Load data
    columns = ['Key','RShoulderPitch', 'RShoulderRoll', 'RElbowRoll']
...
exp= experimentation.Experiment(0.1, 3000, robot)
#exp.run_exp()
